...
Encoder::Encoder(const char* filename, int theWidth, int theHeight, int fps, const char* preset, const char* crf, GLuint id) :
    width { theWidth },
    height { theHeight },
    textureID { id }
{
    ...
    av_dict_set(&formatContext->metadata, "comment", "Kamita", 0);
...
void Encoder::setFrameYUVFromRGB()
{
    ...
    swsContext = sws_getCachedContext(swsContext, codecContext->width, codecContext->height, AV_PIX_FMT_RGB32, codecContext->width, codecContext->height, AVPixelFormat::AV_PIX_FMT_YUV422P, 0, NULL, NULL, NULL);
    sws_scale(swsContext, (const uint8_t* const*)&pixels, inLinesize, 0, codecContext->height, frame->data, frame->linesize);
}

void Encoder::encodeFrame(AVFrame* oneFrame)
{
    setFrameYUVFromRGB();
